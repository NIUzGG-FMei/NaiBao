...
- **内置资源**：安装包和绿色版会自动携带 4 个动作 GIF（位于程序目录 `gifs\`）。程序优先使用设置中指定的路径；路径为空或文件不存在时，自动回退到安装目录里的内置 GIF，**安装的人无需再单独获取素材**。
...
1. 下载 `naibao-portable-1.1.4.zip`。
2. 解压到任意目录。
3. 双击 `naibao.exe` 运行。
...
| 版本 | 日期 | 说明 |
|---|---|---|
| v1.1.4 | 2026-08 | 4 个动作 GIF 内置进安装包/绿色版（`gifs` 目录），自动回退到安装目录资源 |
| v1.1.3 | 2026-08 | 修复 GIF 元数据查询（disposal/偏移），消除叠影并校正动作衔接 |
| v1.1.2 | 2026-08 | 修复泡泡菜单相对坐标定位、GIF 变尺寸子帧合成（CopyPixels 缓冲区错误） |
